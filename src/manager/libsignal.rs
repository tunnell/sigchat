...
mod tests {
    ...
    fn aes256_ctr_keystream_advances_across_block_boundary() {
        ...
        let pt = vec![0u8; 17];
        let ct = aes256_ctr_encrypt(&key, &pt).expect("ok");
        // Byte 16 (the first byte of block 2) is keystream byte 0 of the
        // second AES block; byte 0 is keystream byte 0 of the first AES
        // block. They are computed from different counter values and so
