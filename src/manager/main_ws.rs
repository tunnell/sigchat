...
//! decrypts incoming envelopes using the Phase 2 pddb-backed stores, and logs
//! the plaintext. Phase 3 delivers to UI via logging only.
//!
//! Lifecycle: spawned by `Manager::start_receive()`; runs until the connection
//! drops or an unrecoverable error occurs. Fire-and-forget — no IPC needed.
...
struct EnvelopeProto {
    ...
    #[prost(int32, optional, tag = "1")]
    r#type: Option<i32>,
    // sourceServiceId: the sender's ACI UUID string
    #[prost(string, optional, tag = "2")]
...
impl MainWsWorker {
    /// Spawn the receive worker. Returns immediately; the worker runs until
    /// the server closes the connection or an unrecoverable error occurs.
    pub fn spawn(
        ...
        let t = thread::Builder::new()
            ...
            .spawn(move || worker_loop(aci_service_id, device_id, password, host))
            .map_err(|e| io::Error::other(format!("main_ws spawn: {e}")))?;
        Ok(Self { thread: t })
    }

    /// Block until the worker exits. Provided for clean shutdown; Phase 3
    /// callers normally discard the handle and let the worker run freely.
    #[allow(dead_code)]
    pub fn join(self) {
...
fn worker_loop(aci_service_id: String, device_id: u32, password: String, host: String) {
        ...
        match ws_msg.r#type {
            Some(WS_TYPE_REQUEST) => {
                if let Some(req) = ws_msg.request {
                    handle_request(&mut ws, req, &local_addr);
                }
            }
...
fn handle_request(ws: &mut SignalWS, req: WsRequestProto, local_addr: &ProtocolAddress) {
    ...
    if verb == "PUT" && path == "/api/v1/message" {
        if let Some(body) = req.body {
            dispatch_envelope(body, local_addr);
        }
        send_ack(ws, id, 200);
...
fn dispatch_envelope(body: Vec<u8>, local_addr: &ProtocolAddress) {
    ...
    // Create pddb handles. Each store takes ownership of its handle.
